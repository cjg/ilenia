...
repository_t *httpup_get_repository(driver_t * self, char *supfile,
				    list_t * repositories_hierarchy)
{
	...
	assert(supfile);
	if (!(extension = rindex(supfile, '.')) || strcmp(extension, ".httpup"))
		return NULL;
...
void httpup_update(repository_t * repository)
{
	assert(repository != NULL);
	char *args[] = { "httpup", repository->supfile, NULL };
	exec("/etc/ports/drivers/httpup", args);
}
