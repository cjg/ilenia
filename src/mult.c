...
repository_t *mult_get_repository(driver_t * self, char *supfile,
				  list_t * repositories_hierarchy)
{
	...
	assert(supfile);
	if (!(extension = rindex(supfile, '.')) || strcmp(extension, ".mult"))
		return NULL;
	...
	while ((nread = getline(&line, &n, file)) >= 0) {
		*(line + strlen(line) - 1) = 0;
		...
		if (line[0] == '#' || strlen(line) == 0 ||
		    (!strstr(line, "ROOT_DIR") && strchr(line, '=')))
			continue;
...
void mult_update(repository_t * repository)
{
	FILE *file;
	char *line, *dot_file;
	size_t n;
	...
	file = fopen(repository->supfile, "r");
	if (file == NULL) 
		return warning("Can't open %s\n", repository->supfile);
	...
	while ((nread = getline(&line, &n, file)) >= 0) {
		*(line + strlen(line) - 1) = 0;
		strtrim(line);
		if (line[0] == '#' || strlen(line) == 0 ||
		    (!strstr(line, "DOT_FILE") && strchr(line, '=')))
			continue;
		dot_file = strtrim(xstrdup(strchr(line, '=') + 1));
